      ...
      <DialogPrimitive.Popup
        ...
        {showCloseButton && (
          <DialogPrimitive.Close
            ...
            render={
              <Button variant="ghost" className="absolute top-2 right-2 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-md" size="icon" />
            }>
            <XIcon />
...
function DialogFooter({
  ...
  return (
    <div
      ...
      className={cn(
        "-mx-4 -mb-4 sm:-mx-6 sm:-mb-6 flex flex-col-reverse gap-2 rounded-b-xl border-t bg-muted/50 p-4 sm:p-6 sm:flex-row sm:justify-end",
        className
      )}
      {...props}>
      {children}
      {showCloseButton && (
        <DialogPrimitive.Close render={<Button variant="outline" className="w-full sm:w-auto min-h-[44px]" />}>
          Close
        </DialogPrimitive.Close>
      )}
    </div>
